...
POLICY_NN_MODEL = None  # path of the policy network model, or None. e.g., "Models/policy_sl_ps_worker_100.ckpt"
VALUE_NN_MODEL = None  # path of value network model. e.g., "Models/value_rl_ps_worker_1000.ckpt"
...
if TRAINING_MODE == "SL":
    VAL_INTERVAL = 50
VAL_ON_MASTER = True  # TODO: validation on agent uses CPU instead of GPU, and may cause use up all memory, do not know why, so far it must be set true
...
NUM_RES_TYPES = 2  # number of resource types, e.g., cpu, gpu
NUM_RES_SLOTS = 8  # number of available resource slots on each machine
...
INPUTS_GATE = [("TYPE", True), ("STAY", True), ("PROGRESS", True), ("DOM_RES", True), ("WORKERS", True)]
if PS_WORKER:
